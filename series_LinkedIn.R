...
data = read.csv(".\\data\\daily_linkedin_hva.csv")
...
ss <- AddLocalLinearTrend(list(), ts$form_fills)
ss <- AddSeasonal(ss, train$form_fills, nseasons = 100)
...
CompareBstsModels(model.list = list(local = model1,linear =model2,season = model3,season2=model5,local_3 = model8,local_2 = model9))
...
inclusionprobs <- melt(colMeans(model1$coefficients[-(1:burn),] != 0))
